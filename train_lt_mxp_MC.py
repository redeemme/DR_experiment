...
import operator
import time
import SEmutualchannel
import mutualchannel
...
parser.add_argument('--csv_train', type=str, default='data/IPCL_train_raw_2022O2U.csv', help='path to training data csv')
parser.add_argument('--data_path', type=str, default='/home/user5/balanced_mixup/data/IPCL_2022_1', help='path data')
...
parser.add_argument('--n_classes', type=int, default=7, help='binary disease detection (1) or multi-class (5)')
parser.add_argument('--loss_fn', type=str, default='ce', help='loss function (ce)')
parser.add_argument('--do_mixup', type=float, default=0.3,help='mixup coeff (so far only for multi-class')
...
parser.add_argument('--im_size', help='delimited list input, could be 500, or 600,400', type=str, default='224,224')
...
parser.add_argument('--n_checkpoints', type=int, default=30, help='nr of best checkpoints to keep (defaults to 3)')
    ...
    with trange(len(loader)) as t:
        ...
        for i_batch, batch in enumerate(loader):
            ...
            if train:
                ...
                # # mutual channel loss
                logits, ce_loss, MC_loss = model(inputs, mixed_labels)
                ...
                # mean over the batch
                loss = 0.1*((1 - pt) ** 2 * ce_loss).mean() + 0.9*(ce_loss + args.alpha_1 * MC_loss[0] + args.beta_1 * MC_loss[1])
            else:
                #balanced mixup
                logits = model(inputs,labels)[0]
...
def train_model(model, sampling, optimizer, train_criterion, val_criterion, do_mixup, train_loader, val_loader,
                scheduler, metric, start_epoch,n_epochs, exp_path, n_checkpoints):
...
if __name__ == '__main__':
    ...
    net = SEmutualchannel.model_bn(224, args.n_classes)
    ...
    tr_mccs, vl_mccs, tr_ks, vl_ks, tr_ls, vl_ls, b_epoch = train_model(model, sampling, optimizer, train_criterion, val_criterion,
                                                                        ...
                                                                        metric, start_epoch,n_epochs, experiment_path, n_checkpoints)
