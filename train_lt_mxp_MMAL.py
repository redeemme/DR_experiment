...
from config import proposalN,channels
...
from MMALmodel import MainNet
from dbt_pytorch import *

writer = SummaryWriter('./res_log')
...
parser.add_argument('--patience', type=int, default=10, help='patience before lr reduction')
...
parser.add_argument('--n_epochs', type=int, default=100, help='nr epochs') #
parser.add_argument('--metric', type=str, default='kappa', help='which metric to use for monitoring progress (loss/dice)')
parser.add_argument('--im_size', help='delimited list input, could be 500, or 600,400', type=str, default='448,448')
...
parser.add_argument('--num_workers', type=int, default=1, help='number of parallel (multiprocessing) workers to launch '
...
parser.add_argument('--n_checkpoints', type=int, default=60, help='nr of best checkpoints to keep (defaults to 3)')
parser.add_argument('--mix_up_or_not',type=bool,default=True)
...
def run_one_epoch(loader, model, epoch,criterion, do_mixup=0., optimizer=None, assess=False,mix_up_or_not = False):
    ...
    with trange(len(loader)) as t:
        ...
        for i_batch, batch in enumerate(loader):
            ...
            if train:
                # DBT
                # logits,group_loss = model(inputs)
                # ce_loss =cross_entropy_loss(logits, mixed_labels)
                # loss_func2 = nn.MSELoss()
                # group_loss_ = group_loss * 0
                # loss_matrix = loss_func2(group_loss,group_loss_)
                # loss = ce_loss + loss_matrix
                # # mutual channel loss
                proposalN_windows_score_mix, proposalN_windows_logits_mix, indices_mix, \
                window_scores_mix, _, raw_logits_mix, local_logits_mix, _ = model(inputs, mixed_labels)
                proposalN_windows_score, proposalN_windows_logits, indices, \
                window_scores, _, raw_logits, local_logits, _ = model(inputs, labels)
                logits = raw_logits
                logits_mix = raw_logits_mix
                ...
                raw_loss_mix = criterion(raw_logits, mixed_labels)
                local_loss = criterion(local_logits, labels)
                local_loss_mix = criterion(local_logits_mix,mixed_labels)
                windowscls_loss = criterion(proposalN_windows_logits,
                                            labels.unsqueeze(1).repeat(1, proposalN).view(-1))
                windowscls_loss_mix = criterion(proposalN_windows_logits_mix,
                                            mixed_labels.unsqueeze(1).repeat(1, proposalN).view(-1))

                if epoch < 2:
                    total_loss = (raw_loss + windowscls_loss_mix)/2
                    loss = total_loss
                else:
                    total_loss = ((raw_loss + local_loss + windowscls_loss) + (raw_loss_mix + local_loss_mix + windowscls_loss_mix))/2
                    loss = total_loss


            else:
                #balanced mixup
                logits = model(inputs,labels)[5]
                # loss = cross_entropy_loss(logits, mixed_labels) if train else criterion(logits, labels)
                # logits, group_loss = model(inputs)
                ce_loss = cross_entropy_loss(logits, mixed_labels) if train else criterion(logits, labels)
                loss = ce_loss
            ...
            if assess:
                ...
                # time.sleep(0.1) # resnet50延缓gpu压力
...
def train_model(model, sampling, optimizer, train_criterion, val_criterion, do_mixup, train_loader, val_loader,
                scheduler, metric, start_epoch,n_epochs, exp_path, n_checkpoints,mix_up_or_not):
    ...
    # class_names = ['C{}'.format(i) for i in range(0,7)]
    ...
    for epoch in range(start_epoch,n_epochs):
        ...
        # train one epoch
        _, _, _, _ = run_one_epoch(combo_loader, model,epoch, train_criterion, do_mixup, optimizer, assess=True)

        with torch.no_grad():
            tr_preds, tr_probs, tr_labels, tr_loss = run_one_epoch(train_loader, model,epoch,val_criterion,assess=True,mix_up_or_not=mix_up_or_not)
            vl_preds, vl_probs, vl_labels, vl_loss = run_one_epoch(val_loader, model, epoch,val_criterion, assess=True,mix_up_or_not=mix_up_or_not)
        ...
        accuracy,tr_auc, tr_k, tr_mcc, tr_f1, tr_bacc, tr_auc_all, tr_f1_all = evaluate_multi_cls(tr_labels, tr_preds, tr_probs, print_conf=print_conf,
                                                              class_names=class_names, text_file=text_file_train, loss=tr_loss)
        accuracy,vl_auc, vl_k, vl_mcc, vl_f1, vl_bacc, vl_auc_all, vl_f1_all = evaluate_multi_cls(vl_labels, vl_preds, vl_probs, print_conf=print_conf,
        ...
        writer.add_scalar('tra_auc',tr_auc,epoch)
        writer.add_scalar('vl_auc',vl_auc,epoch)
        writer.add_scalar('tr_mcc',tr_mcc,epoch)
        writer.add_scalar('vl_mcc',vl_mcc,epoch)
        writer.add_scalar('tr_bacc',tr_bacc,epoch)
        writer.add_scalar('vl_bacc',vl_bacc,epoch)
        writer.add_scalar('vl_loss',vl_loss,epoch)
...
if __name__ == '__main__':
    ...
    net = MainNet(proposalN=proposalN, num_classes=n_classes, channels=channels)
    # gather parser parameters
    sampling = args.sampling
    model_name = args.model_name
    optimizer_choice = args.optimizer
    lr, min_lr, patience, bs = args.lr, args.min_lr, args.patience, args.batch_size
    mix_up_or_not = args.mix_up_or_not
    # net = get_resnet_SG_GB()
    ...
    model, mean, std = get_arch(model_name, n_classes=args.n_classes)
    ...
    # #load checkpoint
    # checkpoint = torch.load("/home/user5/balanced_mixup/experiments/eyepacs/mxp_1e-1/mobilenet/epoch_20_K_79.31_mAUC_91.32_MCC_61.69/model_checkpoint.pth")
    ...
    tr_mccs, vl_mccs, tr_ks, vl_ks, tr_ls, vl_ls, b_epoch = train_model(model, sampling, optimizer, train_criterion, val_criterion,
                                                                        ...
                                                                        metric, start_epoch,n_epochs, experiment_path, n_checkpoints,mix_up_or_not)
