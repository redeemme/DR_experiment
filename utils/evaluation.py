...
def evaluate_multi_cls(y_true, y_pred, y_proba, print_conf=True, text_file=None, class_names=None, loss=0, lr=None):
    ...
    print(classification_report(y_true,y_pred)) #输出报告
    ...
    if present_classes != classes:
        ...
        # print(y_true_ohe[:,6])
        # label_columns = ['Histology_' + i for i in
        #                  ['Normal', 'Inflammation', 'LGIN', 'HGIN', 'SM1', 'MM', 'SM2 or deeper']]
        # for i in range(len(present_classes)):
        #     with open("class.txt",'a') as f:
        #         print(label_columns[i],file=f)
        #         print(y_true_ohe[:, i],file=f)
        #         print(y_pred_ohe[:, i],file=f)
        #         print(accuracy_score(y_true_ohe[:, i], y_pred_ohe[:, i]),file=f)
    ...
    if print_conf:
        if text_file is not None:
            ...
            if len(class_names)==3:
                ...
                return accuracy,mean_auc, k, mcc, f1, bacc, auc_all, f1_all
